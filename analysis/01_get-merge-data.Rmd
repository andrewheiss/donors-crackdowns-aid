...
country_level_data <- panel_skeleton %>% 
  ...
  left_join(select(disasters_groups, gwcode, year, starts_with("natural_")), 
            by = c("gwcode", "year")) %>% 
...
country_aid_complete <- country_aid %>% 
  ...
  select(-e_polity2, -v2x_regime_amb)
...
There are only two countries now that have any missing data: 
...
Since those are both pre-1990, our data is perfect post-1990 in cases with Suparna's law coverage:
